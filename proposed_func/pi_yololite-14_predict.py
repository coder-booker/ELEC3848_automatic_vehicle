...
class SerialNode:
    def __init__(self, real=True, IP="192.168.4.1", PORT='/dev/ttyUSB0', BPS=115200):
    ...
    def _check(self):
        if self.available and self.ser.isOpen():
            print(f"Serial successful. a:{self.available}|s:{self.ser.isOpen()}")
            return True
        else:
            print(f"Port not available. a:{self.available}|s:{self.ser.isOpen()}")
            return False

    def write_data(self, data="Hello I am Ras PI"):
        if self._check():
...
class PostProcessor():
    ...
    def process(self, frame, detected, boxes, confs, ids, dic_labels, predict_time):
        ...
        if detected:
            ...
            print("No target. ")
            data_to_be_write = '000\r'
        
        self.usb_serial.write_data(data_to_be_write)
        rev_data = self.usb_serial.read_data()
            
        str_FPS = "FPS: %.2f"%(1./(time.time() - t1 + 0.0001))
